...
class StrategyOptions():
    ...
    def enabled_strategies(self) -> dict[str,Strategy]:
        ...
        """
        return {prop:strategy for (prop, strategy)
                in vars(self).items() if strategy and strategy.enabled}
    ...
    def chosen_strategy(self) -> tuple[str,Strategy]:
        ...
        """
        return next(
            ((prop, strategy) for (prop, strategy)
             in vars(self).items() if strategy and strategy.chosen),
            None)
...
class User(BaseModel):
    ...
    def state_supported(cls, state):
        ...
        if state not in STATE_BRACKET_RATES:
            raise ValueError(f'{state} is not supported. You can add it to data/taxes.py!')
        return state
