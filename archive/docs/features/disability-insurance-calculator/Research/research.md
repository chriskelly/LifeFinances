...
- Includes dates for age-based calculations (until policy end, configurable, default 65)
...
### Q3: How to handle post-policy end income reductions (reduced Social Security)?
...
- Difference = post-policy end reduction amount to include in total replacement needs
...
- Post-policy end reductions are naturally included in the lifetime Social Security totals

**Alternatives considered**:
- Manual Social Security calculation - **Rejected** as SimulationEngine handles this correctly
- Ignoring post-policy end reductions - **Rejected** per spec requirement
...
- Coverage duration may extend beyond working years (until policy end, configurable, default 65)
...
1. Calculate total income replacement needs:
   - Lost job income until policy end (`age_limit` or `duration_years` in config)
   - Reduced Social Security (baseline SS - disability SS, including post-policy end)
   - Reduced pension (baseline pension - disability pension, including post-policy end)
2. Calculate existing coverage replacement (net after taxes)
3. Coverage gap = Total needs - Existing coverage
4. Benefit percentage = (Coverage gap / Years until policy end) / Current annual income
   - Current annual income = first income profile with income > $0

**Rationale**:
- Matches spec requirements for gap calculation
- Includes post-policy end reductions in total needs
...
- Calculate years until policy end from user age (using configurable policy end, default 65)
