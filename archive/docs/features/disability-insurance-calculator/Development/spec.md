...
- Q: How should coverage gap be calculated when coverage duration extends beyond working years? → A: Calculate gap to replace total future post-tax income user would have earned. Policy end for employer benefits and the gap % denominator comes from each person's `DisabilityCoverage` in config: `age_limit` (until that age) or `duration_years` (from simulation start). Total replacement needs = (Baseline post-tax income - Disability post-tax income), where post-tax income = (Job Income + Social Security + Pension) - (Income Taxes + Medicare Taxes). This includes lost post-tax job income + reduced post-tax Social Security + reduced post-tax pension (including post-policy-end reductions). Coverage gap = Total replacement needs - Existing coverage replacement. Benefit percentage = (Coverage gap / years until policy end) / Current annual income.
...
- Q: Should coverage gap include income reductions after policy end (such as reduced Social Security)? → A: Yes, coverage gap MUST include post-policy-end income reductions (e.g., if Social Security expected $500k but reduced to $400k due to disability, the $100k reduction is included in total income to be replaced, which feeds into the income replacement rate output)
...
1. **Given** a user with job income that contributes to Social Security eligibility, **When** disability eliminates job income, **Then** the script calculates reduced Social Security benefits (including post-policy-end income reductions) and includes this reduction in total income replacement needs
...
3. **Given** a user with employer-provided coverage where coverage duration extends beyond expected working years, **When** the script calculates existing coverage replacement, **Then** the script applies coverage for the full duration period (up to policy end if applicable)
...
- **Coverage duration extends beyond working years**: Long-term disability (LTD) policy end comes from each person's `DisabilityCoverage` (`age_limit` or `duration_years`). Coverage gap calculation MUST ensure total future post-tax income (including post-policy-end income reductions) is protected, regardless of planned early retirement. Total replacement needs = (Baseline post-tax income - Disability post-tax income), where post-tax income = (Job Income + Social Security + Pension) - (Income Taxes + Medicare Taxes). This includes: (1) Post-tax income until policy end, (2) Post-policy-end income reductions (e.g., if Social Security expected $500k but reduced to $400k, include $100k reduction). Coverage gap = Total replacement needs - Existing coverage replacement. Benefit percentage is calculated as: (Coverage gap / years until policy end) / Current annual income. Coverage periods do NOT need to align with income profile intervals.
...
- **FR-004a**: Script MUST read policy end from each person's `DisabilityCoverage` in the user config: `age_limit` (benefits until insured reaches that age) or `duration_years` (benefit period from simulation start). The same policy end drives employer-benefit modeling, years-until for the gap formula, and recommended duration in output.
- **FR-005**: Script MUST calculate total income replacement needed for user if user becomes disabled, using post-tax income values. Total replacement needs = (Baseline post-tax income - Disability post-tax income), where post-tax income = (Job Income + Social Security + Pension) - (Income Taxes + Medicare Taxes). This includes:
  - Lost post-tax job income for remaining working years until policy end (from `DisabilityCoverage.age_limit` or `duration_years` in config)
  - Reduced post-tax Social Security benefits due to lost earnings history (including post-policy-end income reductions, e.g., if expected $500k but reduced to $400k, include $100k reduction)
  - Reduced post-tax pension benefits due to fewer years worked and/or lower final compensation (including post-policy-end income reductions)
  - Post-policy end income reductions (e.g., reduced Social Security benefits after policy end) must be included in total income to be replaced
- **FR-006**: Script MUST calculate total income replacement needed for partner if partner becomes disabled, using the same calculation method as FR-005 but applied to partner's income profiles and benefits. Total replacement needs = (Baseline post-tax income - Disability post-tax income), where post-tax income = (Job Income + Social Security + Pension) - (Income Taxes + Medicare Taxes). This includes:
  - Lost post-tax partner job income for remaining working years until policy end (from `DisabilityCoverage.age_limit` or `duration_years` in config)
  - Reduced post-tax partner Social Security benefits due to lost earnings history (including post-policy-end income reductions)
  - Reduced post-tax partner pension benefits due to fewer years worked and/or lower final compensation (including post-policy-end income reductions)
  - Post-policy end income reductions must be included in total income to be replaced
...
- **FR-008**: Script MUST calculate remaining coverage gap to ensure total future post-tax income (including post-policy-end income reductions) is protected. Total replacement needs is calculated as the difference between baseline and disability post-tax income, where post-tax income = (Job Income + Social Security + Pension) - (Income Taxes + Medicare Taxes). This includes: (1) Lost post-tax job income until policy end, (2) Reduced post-tax Social Security benefits (lifetime, including post-policy-end reductions), and (3) Reduced post-tax pension benefits (lifetime, including post-policy-end reductions). For example, if Social Security expected $500k but reduced to $400k, include the $100k reduction in the sum. Coverage gap = Total replacement needs - Existing coverage replacement. Gap calculation formula: Benefit percentage = (Coverage gap / Years until policy end) / Current annual income. Current annual income is determined as the first income profile with income over $0 (user may be on sabbatical with $0 current income). Size benefit to make up for missing future income after accounting for existing coverage replacement (capped at 100% of income). Gap cannot be negative (minimum 0%).
...
- **FR-020**: Script MUST calculate coverage needs until policy end (from `DisabilityCoverage.age_limit` or `duration_years` in config), regardless of planned early retirement. Coverage gap is sized to replace total future income that would have been earned until policy end, including post-policy-end income reductions as specified in FR-008 (e.g., reduced Social Security benefits after policy end).
...
- **SC-002**: Script produces accurate income replacement calculations that account for all income sources (job until policy end, Social Security including post-policy-end income reductions, pension including post-policy-end income reductions) for both user and partner scenarios
...
- **SC-006**: Secondary effect calculations (reduced Social Security and pension, including post-policy-end income reductions) reflect realistic impact of lost job income on future benefits. Post-policy end income reductions (e.g., reduced Social Security) are included in total income to be replaced.
...
- Disability is assumed to occur immediately and last until policy end (from `DisabilityCoverage.age_limit` or `duration_years` in config)
- Long-term disability insurance typically covers until policy end (from `DisabilityCoverage.age_limit` or `duration_years` in config), regardless of planned early retirement
- policy end is configurable in the script (default value is 65, but can be set to a different age)
- Coverage gap calculation seeks to replace total future income including post-policy-end income reductions (e.g., reduced Social Security benefits after policy end), not just income until policy end. See FR-008 for detailed calculation formula.
