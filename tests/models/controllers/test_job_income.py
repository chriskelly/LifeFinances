...
def test_job_income_controller(sample_user: User):
    ...
    expected_total_income = [
        user + partner
        for user, partner in zip(expected_user_income, expected_partner_income, strict=True)
    ]
    expected_tax_deferred = [2.5, 2.5, 2.75, 2.75, 2.75, 11.25]
    expected_taxable_income = [
        total - deferred
        for total, deferred in zip(expected_total_income, expected_tax_deferred, strict=True)
    ]
