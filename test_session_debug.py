...
def test_session_flow():
    ...
    with app.test_client() as client:
        ...
        print(f"   Redirect location: {response.location if response.status_code == 302 else 'N/A'}")

        # Step 2: Check session after simulation
        with client.session_transaction() as sess:
            print("\n2. Session contents after simulation:")
            print(f"   - first_results_data: {sess.get('first_results_data') is not None}")
            if sess.get('first_results_data'):
                ...
                print(f"     Sample: {sess.get('first_results_data')[0] if sess.get('first_results_data') else 'N/A'}")

            print(f"   - first_results_columns: {sess.get('first_results_columns')}")
            print(f"   - success_percentage: {sess.get('success_percentage')} (type: {type(sess.get('success_percentage'))})")
            print(f"   - simulation_history: {len(sess.get('simulation_history', []))} entries")
        ...
        if response.status_code == 200:
            ...
            print(f"   Contains 'No Results Available': {'No Results Available' in data}")
        ...
        with client.session_transaction() as sess:
            ...
            print(f"   - first_results_data: {sess.get('first_results_data') is not None}")
            print(f"   - first_results_columns: {sess.get('first_results_columns') is not None}")
