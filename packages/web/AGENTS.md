...
- **Density** (overall size): edit `FONT_SIZE`, `LINE_HEIGHT`, `SPACING`, `FORM_SPACING_VERTICAL`, and `FORM_SPACING_HORIZONTAL` in `theme.py`. Defaults are “slightly denser” than stock Pico. For noticeably compact, try `FONT_SIZE = "87.5%"`, `FORM_SPACING_VERTICAL = "0.35rem"`, `FORM_SPACING_HORIZONTAL = "0.65rem"`. Shell gaps in `static/style.css` (layout/`editor-pane` padding) are separate if the page still feels roomy.
