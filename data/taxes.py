...
def get(income_qt:float, residence_state:str, inflation:float, married:bool) -> float:
    """Combines federal and state taxes on non-tax-deferred income

    Args:
        income_qt (float): income in quarterly amount
        residence_state (string): state of residence for tax purposes
    ...
    fed_taxes = bracket_math(FED_BRACKET_RATES[married],
                             max(adj_income - FED_STD_DEDUCTION[married], 0))
    state_taxes = bracket_math(STATE_BRACKET_RATES[residence_state][married],
                               max(adj_income - STATE_STD_DEDUCTION[residence_state][married], 0))
